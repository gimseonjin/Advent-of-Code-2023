...
class Game {
  ...
  set(color: string, amount: number) {
    if (color === "red") this.red = Math.max(this.red, amount);
    else if (color === "blue") this.blue = Math.max(this.blue, amount);
    else if (color === "green") this.green = Math.max(this.green, amount);
  }

  isPossible() {
    return (
      this.red <= MAX_RED_CUBES &&
      this.blue <= MAX_BLUE_CUBES &&
      this.green <= MAX_GREEN_CUBES
    );
  }
...
async function main() {
  const result = (await readFileLines(__dirname, "input.txt"))
    ...
    .reduce((accumulator, game) => accumulator + game.getId(), 0);
