...
function calculateSumOfAdjacentNumbers(
  ...
  return symbolCoordinateList
    .map((symbolCoordinate) =>
      findNumbersAroundSymbol(symbolCoordinate, checked, engineSchematic)
    )
    .map((adjacentNumbers) => adjacentNumbers.filter((number) => number !== 0))
    .filter((adjacentNumbers) => adjacentNumbers.length === 2)
    .map(
      (filteredAdjacentNumbers) =>
        filteredAdjacentNumbers[0] * filteredAdjacentNumbers[1]
    );
}
...
async function main() {
  ...
  const result = adjacentNumbers.reduce((pre, cur) => pre + cur, 0);
